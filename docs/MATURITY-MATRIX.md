...
| A2A, auth-web, auth-fides-tap, x402, and OSP profiles | Draft | These profiles now have explicit mapping notes and fixture coverage, but they are not yet backed by dedicated end-to-end runtimes. | `profiles/`, profile fixture packs |
...
| JSON-RPC, gRPC, and event/webhook binding families | Concept | Mentioned in architecture and roadmap, but not specified or implemented enough to claim as current surfaces. | `docs/SUITE-ARCHITECTURE.md`, `ROADMAP.md` |
