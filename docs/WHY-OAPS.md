...
AICP exists because autonomous systems keep needing the same control-plane primitives across many different protocols, and those primitives are still handled inconsistently.
...
- **ACP** and **UCP** are valuable commerce ecosystems, but AICP should not turn into a checkout product spec.
- **OSP** is a strong provisioning alignment target, but provisioning semantics should still be expressed in AICP terms.

## What AICP Adds

AICP adds a common semantic layer for things multiple ecosystems keep re-inventing:
...
AICP should not be described as a wrapper around MCP, A2A, x402, ACP, UCP, or OSP.
...
AICP is intended to stay neutral by design.
